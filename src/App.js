        ...
        <div>
            ...
            <About />
           {
            sectionData.map(value =>{
                return(
                    //return() this should be used if we use {}
                <Section 
                x={value.title}
                y={value.des}
                />)
...
//ifelse is not allowed here so only TERNARY operator is allowed
//loops =>only higher order functions allowed => map,filter, reduce